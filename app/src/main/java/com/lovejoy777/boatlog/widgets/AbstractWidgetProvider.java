package com.lovejoy777.boatlog.widgets;
...
public abstract class AbstractWidgetProvider extends AppWidgetProvider {
    ...
    protected Weather parseWidgetJson(String result, Context context) {
        try {
            WeatherMainActivity.initMappings();
            ...
            if (lastUpdateTimeInMillis < 0) {
                ...
                lastUpdate = context.getString(R.string.last_update_widget, WeatherMainActivity.formatTimeWithDayIfNotToday(context, lastUpdateTimeInMillis));
            }
            ...
            widgetWeather.setWind(context.getString(R.string.wind) + ": " + new DecimalFormat("#.0").format(wind) + " " + localize(sp, context, "speedUnit", "m/s")
                    + (widgetWeather.isWindDirectionAvailable() ? " " + WeatherMainActivity.getWindDirectionString(sp, context, widgetWeather) : ""));
    ...
    protected String localize(SharedPreferences sp, Context context, String preferenceKey,
        ...
        return WeatherMainActivity.localize(sp, context, preferenceKey, defaultValueKey);
    }
